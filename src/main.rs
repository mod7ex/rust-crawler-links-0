...
use std::{ borrow::Borrow, future::Future, pin::Pin, error::Error };

use async_std::task;
use surf;

type CrawlResult = Result<(), Box<dyn Error + Send + Sync + 'static>>;

type BoxFuture = Pin<Box<dyn Future<Output = CrawlResult> + Send>>;
